...
impl XousHidConnection {
    pub fn recv_with_timeout(&mut self, buf: &mut [u8; 64], timeout_delay: Duration) -> SendOrRecvStatus {
        match self.endpoint.read_report() {
            Ok(msg) => {
                buf.copy_from_slice(&msg.0);
    ...
    pub fn u2f_wait_incoming(&self) -> Result<HIDReport, xous::Error> { self.endpoint.read_report() }

    pub fn u2f_send(&self, msg: HIDReport) -> Result<(), xous::Error> { self.endpoint.write_report(msg) }
}
...
impl HidConnection for XousHidConnection {
    fn send_and_maybe_recv(&mut self, buf: &mut [u8; 64], _timeout: Duration) -> SendOrRecvResult {
        ...
        reply.0.copy_from_slice(buf);
        match self.endpoint.write_report(reply) {
...
impl XousEnv {
    ...
    pub fn new(conn: xous::CID, lefty_mode: Arc<AtomicBool>) -> Self {
        ...
        let usbhid_conn = usb_device_xous::UsbHid::new();
        usbhid_conn.connect_hid_app(FIDO_REPORT_DESCRIPTOR.to_vec()).unwrap();
        usbhid_conn.switch_to_core(usb_device_xous::UsbDeviceType::HIDv2).unwrap();

        XousEnv {
            rng: XousRng256::new(&xns),
            store,
            main_connection: XousHidConnection { endpoint: usbhid_conn },
            #[cfg(feature = "vendor_hid")]
...
impl UserPresence for XousEnv {
    ...
    fn poll_approval_ctap1(&mut self, reason: String, app_id: [u8; 32]) -> bool {
        ...
        let request = Ctap1Request { reason: xous_ipc::String::from_str(&reason), app_id, approved: false };
